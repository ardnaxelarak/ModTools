...
CREATE OR REPLACE VIEW game_view AS select g.gid, t.tid, t.short_name AS type_short, t.name AS type_long, s.sid, s.name AS status_name, s.signup, (s.rooms AND t.rooms AND NOT g.rooms_hidden) AS show_rooms, g.thread_id, g.game_index, g.name AS game_name, count(DISTINCT p.pid) AS current_players, g.max_players, g.rooms_hidden, g.roles_change FROM games g LEFT JOIN game_types t ON g.tid = t.tid LEFT JOIN statuses s ON g.status = s.sid LEFT JOIN game_players p ON p.gid = g.gid GROUP BY g.gid;
